...
(function (window) {
     ...
     * @constructor
     */
    var InfiniteTLChart = function (element, recordset, plotData) {
...
//        console.log('InfiniteTLChart.initialize()');
        ...
        this.canvas.width = parseInt(element.style.width, 10);
        this.canvas.height = parseInt(element.style.height, 10) - this.titleDiv.offsetHeight;
    ...
    p.drawChart = function () {
        ...
        yLabel = 100;
        for (i = 0; i < 5; i++) {
            this.chartContext.fillText((yLabel === 100 ? '' : '  ') + yLabel + 't', 0, (this.yTicks * i + this.chartGutterTop + 5));
            yLabel -= 20;
        }
        //add x labels
        ...
        this.chartContext.strokeStyle = '#ffffff';
     ...
     */
    p.plotData = function (chartDiv, currentDateInterval, pxPerDay, dataProvider) {
        var i, d, div, x,
            amount = 0,
    ...
    p.createYdivs = function () {
        var div, mDown, tStart, tMove, tEnd,
            i = 0,
            that = this,
            l = this.xDensity,
        ...
        if (noExistingDivs < l) {
            ...
            tStart = function (event) { that.dragTouchStart(event); };
            tMove = function (event) { that.dragTouchMove(event); };
            tEnd = function (event) { that.dragTouchEnd(event); };
            mDown = function (event) { that.onYDivMouseDown(event); };
            ...
            for (i; i < l; i++) {
                ...
                // attach handlers
                // events for touch
                div.addEventListener('touchstart', tStart, false);
                div.addEventListener('touchmove',  tMove,  false);
                div.addEventListener('touchend',   tEnd,   false);
                // events for mouse
    ...
    p.updateChartTitle = function (total, startDate, endDate) {
        ...
        this.titleDiv.innerHTML = 'Reserve Transactions from ' + this.formatSingleDate(startDate) + ' to ' + this.formatSingleDate(endDate) + ' (' + days + ' days)<br>' + 'Projected Revenue: $' + this.addCommas(total);
    };
    ...
    p.createNewDiv = function (data) {
        var div,
            that = this;
        div = document.createElement('div');
        div.style.backgroundColor = '#55ffa9'; //#cea746
    ...
    p.showTooltip = function (e) {
        ...
        this.toolTipView.innerHTML = '<strong>Project: <nobr>' + m.project + '</nobr></strong><br/>Lot#: ' + m.account + '<br/>Value: $' + this.addCommas(m.revenue) + '<br/>Timestamp: ' + d + '<br/>Units: ' + m.probability + '<br/>Buyer: J&F Investments';
...
*****************************************************************/
    ...
    p.getDummyData = function () {
        return [
            {
                project: 'Brazil Potash',
                account: '28741',
                revenue:  250000,
                closeDate: new Date(2018, 5, 1),
                probability: 30,
                div: undefined
            },
            {
