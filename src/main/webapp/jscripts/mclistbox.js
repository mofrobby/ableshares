...
//alert("BuildMCListBox ")
	...
	this.unhighlightAll = mtdUnhighlightAll
...
function mtdSwitchSelect(row) {
	...
	for (var i = this.headCount; i < this.table.rows.length; i++) {
		if (this.table.rows[i] == row) {
			//alert('this.table.rows[i] equal '+this.table.rows[i]);
...
function mtdHighlight(row) {	
	...
	//var rid = this.table.rows[row].id;
	//alert('rid '+rid);
	//alert('table '+this.table);
	//alert('row# '+row);
	//alert('row '+this.table.rows[row]);
	//var x = document.getElementById(rid);
	//x.style.backgroundColor = '#FFFFCC';
	//alert('highlighted '+row);

    this.table.rows[row].style.background = this.highlightColor;
	//alert('change '+this.table.rows[row].style.background);
}
